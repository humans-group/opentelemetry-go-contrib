...
- Add the new `github.com/humans-group/opentelemetry-go-contrib
/instrgen` package to provide auto-generated source code instrumentation. (#3068, #3108)

## [1.16.0/0.41.0/0.10.0] - 2023-04-28

### Added

- AWS SDK add `rpc.system` attribute in `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/github.com/aws/aws-sdk-go-v2/otelaws`. (#3582, #3617)

### Changed

- Update `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/google.golang.org/grpc/otelgrpc` to align gRPC server span status with the changes in the OpenTelemetry specification. (#3685)
- Adding the `db.statement` tag to spans in `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/go.mongodb.org/mongo-driver/mongo/otelmongo` is now disabled by default. (#3519)

### Fixed

- The error received by `otelecho` middleware is then passed back to upstream middleware instead of being swallowed. (#3656)
- Prevent taking from reservoir in AWS XRay Remote Sampler when there is zero capacity in `github.com/humans-group/opentelemetry-go-contrib
/samplers/aws/xray`. (#3684)
- Fix `otelhttp.Handler` in `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/net/http/otelhttp` to propagate multiple `WriteHeader` calls while persisting the initial `statusCode`. (#3580)

## [1.16.0-rc.2/0.41.0-rc.2/0.10.0-rc.2] - 2023-03-23

### Added

- The `WithPublicEndpoint` and `WithPublicEndpointFn` options in `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/github.com/emicklei/go-restful/otelrestful`. (#3563)

### Fixed

- AWS SDK rename attributes `aws.operation`, `aws.service` to `rpc.method`,`rpc.service` in `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/github.com/aws/aws-sdk-go-v2/otelaws`. (#3582, #3617) 
- AWS SDK span name to be of the format `Service.Operation` in `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/github.com/aws/aws-sdk-go-v2/otelaws`. (#3582, #3521)
- Prevent sampler configuration reset from erroneously sampling first span in `github.com/humans-group/opentelemetry-go-contrib
/samplers/jaegerremote`. (#3603, #3604)
...
- Add `SpanNameFormatter` option to package `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/github.com/gin-gonic/gin/otelgin`. (#3343)

### Changed

- Change to use protobuf parser instead of encoding/json to accept enums as strings in `github.com/humans-group/opentelemetry-go-contrib
/samplers/jaegerremote`. (#3183)

### Fixed

- Remove use of deprecated `"math/rand".Seed` in `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/github.com/Shopify/sarama/otelsarama/example/producer`. (#3396)
- Do not assume "aws" partition in ecs detector to prevent panic in `github.com/humans-group/opentelemetry-go-contrib
/detectors/aws/ecs`. (#3167)
- The span name of producer spans from `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/github.com/Shopify/sarama/otelsarama` is corrected to use `publish` instead of `send`. (#3369)
- Attribute types are corrected in `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/github.com/aws/aws-sdk-go-v2/otelaws`. (#3369)
...
- Change `runtime.uptime` instrument in `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/runtime` from `Int64ObservableUpDownCounter` to `Int64ObservableCounter`,
...
- The GCE detector in `github.com/humans-group/opentelemetry-go-contrib
/detectors/gcp` includes the "cloud.region" attribute when appropriate. (#3367)

## [1.13.0/0.38.0/0.7.0] - 2023-01-30

### Added

- Add `WithSpanNameFormatter` to `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/github.com/gorilla/mux/otelmux` to allow customizing span names. (#3041)
- Add missing recommended AWS Lambda resource attributes `faas.instance` and `faas.max_memory` in `github.com/humans-group/opentelemetry-go-contrib
/detectors/aws/lambda`. (#3148)
- Improve documentation for `github.com/humans-group/opentelemetry-go-contrib
/samplers/jaegerremote` by providing examples of sampling endpoints. (#3147)
- Add `WithServerName` to `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/net/http/otelhttp` to set the primary server name of a `Handler`. (#3182)

### Changed

- Remove expensive calculation of uncompressed message size attribute in `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/google.golang.org/grpc/otelgrpc`. (#3168)
...
- The `WithLogger` option to `github.com/humans-group/opentelemetry-go-contrib
/samplers/jaegerremote` to allow users to pass a `logr.Logger` and have operations logged. (#2566)
- Add the `messaging.url` & `messaging.system` attributes to all appropriate SQS operations in the `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/github.com/aws/aws-sdk-go-v2/otelaws` package. (#2879)
- Add example use of the metrics signal to `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/net/http/otelhttp/example`. (#2610)
- [otelgin] Add support for filters to the `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/github.com/gin-gonic/gin/otelgin` package to provide the way to control which inbound requests are traced. (#2965, #2963)

### Fixed

- Set the status_code span attribute even if the HTTP handler hasn't written anything. (#2822)
- Do not wrap http.NoBody in `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/net/http/otelhttp`, which fixes handling of that special request body. (#2983)
...
- Avoid getting a new Tracer for every RPC in `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/google.golang.org/grpc/otelgrpc`. (#2835)
- Conditionally compute message size for tracing events using proto v2 API rather than legacy v1 API in `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/google.golang.org/grpc/otelgrpc`. (#2647)

### Deprecated

- The `Inject` function in `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/google.golang.org/grpc/otelgrpc` is deprecated. (#2838)
- The `Extract` function in `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/google.golang.org/grpc/otelgrpc` is deprecated. (#2838)
...
- Rename the `Typ` field of `"github.com/humans-group/opentelemetry-go-contrib
/instrumentation/google.golang.org/grpc/otelgrpc".InterceptorInfo` to `Type`. (#2688)
...
- Add gRPC trace `Filter` to the `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/google.golang.org/grpc/otelgrpc` package to provide the way to filter the traces automatically generated in interceptors. (#2572)
- The `TextMapPropagator` function to `github.com/humans-group/opentelemetry-go-contrib
...
- The `github.com/humans-group/opentelemetry-go-contrib
/propagators/autoprop` package to provide configuration of propagators with useful defaults and envar support. (#2258)
...
- Added the `github.com/humans-group/opentelemetry-go-contrib
/samplers/jaegerremote` package.
  This package implements the Jaeger remote sampler for OpenTelemetry Go. (#936)
- DynamoDB spans created with the `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/github.com/aws/aws-sdk-go-v2/otelaws` package now have the appropriate database attributes added for the operation being performed.
...
- Add OpenTracing baggage extraction to the OpenTracing propagator in `github.com/humans-group/opentelemetry-go-contrib
/propagators/ot`. (#1880)
...
- The deprecated `github.com/humans-group/opentelemetry-go-contrib
/exporters/metric/datadog` module is removed. (#1920)
- The deprecated `github.com/humans-group/opentelemetry-go-contrib
/exporters/metric/dogstatsd` module is removed. (#1920)
- The deprecated `github.com/humans-group/opentelemetry-go-contrib
/exporters/metric/cortex` module is removed.
...
- Add `WithClientTrace` option to `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/net/http/otelhttp`. (#875)

### Changed

- All metric instruments from the `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/runtime` package have been renamed from `runtime.go.*` to `process.runtime.go.*` so as to comply with OpenTelemetry semantic conventions. (#1549)

### Fixed

- Change the `http-server-duration` instrument in `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/net/http/otelhttp` to record milliseconds instead of microseconds.
  This changes fixes the code to comply with the OpenTelemetry specification. (#1414, #1537)
- Fixed the region reported by the `"github.com/humans-group/opentelemetry-go-contrib
/detectors/gcp".CloudRun` detector to comply with the OpenTelemetry specification.
  It no longer includes the project scoped region path, instead just the region. (#1546)
- The `"github.com/humans-group/opentelemetry-go-contrib
/instrumentation/net/http/otelhttp".Transport` type now correctly handles protocol switching responses.
...
- The `github.com/humans-group/opentelemetry-go-contrib
/exporters/metric/datadog` module is deprecated. (#1639)
- The `github.com/humans-group/opentelemetry-go-contrib
/exporters/metric/dogstatsd` module is deprecated. (#1639)
- The `github.com/humans-group/opentelemetry-go-contrib
/exporters/metric/cortex` module is deprecated.
...
- The `github.com/humans-group/opentelemetry-go-contrib
/exporters/metric/dogstatsd` exporter no longer support exporting histogram or exact data points. (#1639)
- The `github.com/humans-group/opentelemetry-go-contrib
/exporters/metric/datadog` exporter no longer support exporting exact data points. (#1639)
...
- The `"github.com/humans-group/opentelemetry-go-contrib
/detector/aws/ecs".Detector` no longer errors if not running in ECS. (#1428)
...
- `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/github.com/aws/aws-lambda-go/otellambda/xrayconfig`
...
- The `Transport`, `Handler`, and HTTP client convenience wrappers in the `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/net/http/otelhttp` package now use the `TracerProvider` from the parent context if one exists and none was explicitly set when configuring the instrumentation. (#873)
...
- The `github.com/humans-group/opentelemetry-go-contrib
/propagators/ot` propagator returns the words `true` or `false` for the `ot-tracer-sampled` header instead of numerical `0` and `1`. (#1358)
...
- Split `github.com/humans-group/opentelemetry-go-contrib
/propagators` module into `b3`, `jaeger`, `ot` modules. (#985)
- `otelmongodb` span attributes, name and span status now conform to specification. (#769)
- Migrated EC2 resource detector support from root module `github.com/humans-group/opentelemetry-go-contrib
/detectors/aws` to a separate EC2 resource detector module `github.com/humans-group/opentelemetry-go-contrib
/detectors/aws/ec2` (#1017)
...
- The `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/go.mongodb.org/mongo-driver/mongo/otelmongo` instrumentation now accepts a `WithCommandAttributeDisabled`,
...
- The `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/google.golang.org/grpc/otelgrpc` package instrumentation no longer accepts a `Tracer` as an argument to the interceptor function.
   Instead, a new `WithTracerProvider` option is added to configure the `TracerProvider` used when creating the `Tracer` for the instrumentation. (#373)
- The `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/gopkg.in/macaron.v1/otelmacaron` instrumentation now accepts a `TracerProvider` rather than a `Tracer`. (#374)
- Remove `go.opentelemetry.io/otel/sdk` dependency from instrumentation. (#381)
- Use `httpsnoop` in `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/github.com/gorilla/mux/otelmux` to ensure `http.ResponseWriter` additional interfaces are preserved. (#388)

### Fixed

- The `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/github.com/labstack/echo/otelecho.Middleware` no longer sends duplicate errors to the global `ErrorHandler`. (#377, #364)
- The import comment in `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/net/http/otelhttp` is now correctly quoted. (#379)
...
- Rename `OTelBeegoHandler` to `Handler` in the `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/github.com/astaxie/beego/otelbeego` package. (#359)
- Replace `WithTracer` with `WithTracerProvider` in the `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/gopkg.in/macaron.v1/otelmacaron` instrumentation. (#374)
...
- The `github.com/humans-group/opentelemetry-go-contrib
/instrumentation/google.golang.org/grpc` module has been added to replace the instrumentation that had previoiusly existed in the `go.opentelemetry.io/otel/instrumentation/grpctrace` package. (#189)
